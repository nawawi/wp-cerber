			...
			if ( is_serialized( $values ) ) {
				if ( ! empty( $scan['compressed'] ) && extension_loaded( 'zlib' ) ) {
					if ( PHP_VERSION_ID >= 70000 ) {
						$values = unserialize( gzuncompress( hex2bin( implode( '', $values ) ) ), array( 'allowed_classes' => false ) );
					} else {
						$values = unserialize( gzuncompress( hex2bin( implode( '', $values ) ) ) );
					}
				} else {
					if ( PHP_VERSION_ID >= 70000 ) {
						$values = unserialize( implode( '', $values ), array( 'allowed_classes' => false ) );
					} else {
...
function cerber_get_ip_patterns() {
	return array(
		array(
			...
			'(?:[0-9]{1,3}\.){3}[0-9]{1,3}',
			6,
			'A suspicious external IPv4 address found. Can cause data leakage.',
			'func' => '_is_ip_external',
		),
		array(
			'IPV6',
			2,
			...
			6,
			'A suspicious external IPv6 address found. Can cause data leakage.',
			'func' => '_is_ip_external',
		),
	);
}
...
function cerber_get_ht_patterns() {
	...
	$ret = array(
		...
		array(
			...
			'A suspicious external IPv4 address found. Can cause data leakage.',
			'func'      => '_is_ip_external',
			'not_regex' => '^(Deny from|Allow from|Require)\s+.+',
		),
		array(
			'IPV6',
			2,
		...
		array(
			...
			'(https?:\/\/[^\s]+\.?)',
			6,
			'A suspicious redirection to another, probably phishing website.',
			'func' => '_is_unsafe_redirect_rule',
		),
		array(
			'RFTP',
			2,
		...
		array(
			...
			'php_value\s+(.+)',
			10,
			'An unsafe, suspicious PHP configuration command. Normally must not be here.',
			'func' => '_is_unsafe_php_value',
		),
	);
...
function cerber_add_file_info( $file ) {
	...
	if ( ! cerber_db_query(
		'UPDATE ' . cerber_get_db_prefix() . CERBER_SCAN_TABLE . ' SET '
							. $update_file_name
							. ' file_hash = "' . $file_hash
							. '", file_md5 = "' . $file_md5
							. '", file_size = ' . $size
...
function cerber_get_prev_scan_id( $scan_id = 0 ) {
	...
	if ( ! $scans = cerber_db_get_results(
		'SELECT * FROM ' . cerber_get_db_prefix() . CERBER_SETS_TABLE
...
function cerber_quarantine_file( $file_name, $scan_id, $move = true ) {
	...
	if ( ! file_exists( $restore ) ) {
		...
		fwrite(
			$f,
			'Information for restoring files.' . PHP_EOL
...
function cerber_get_vulnerabilities( $plugin_slug, $plugin ) {
	...
	if ( ! $vu_list
		...
		if ( ! $vu_list = lab_api_send_request(
			array(
				'get_vu_list' => array(
					'plugins' => $plugins,
				),
...
final class CRB_Scan_Grinder {
	...
	private static function iterator( $file_processor, $file_type = null, $scan_status = array( 0, CERBER_UOP ) ) {
		...
		if ( ! $files = cerber_db_get_results(
			'SELECT ' . $calc . ' * FROM ' . cerber_get_db_prefix() . CERBER_SCAN_TABLE .
