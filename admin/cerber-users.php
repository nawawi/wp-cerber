			...
			<fieldset>
				...
				<label for="crb_user_blocked">
					<input name="crb_user_blocked" type="checkbox" id="crb_user_blocked"
					 ...
					 />
					<?php
					_e( 'User is not permitted to log into the website', 'wp-cerber' );
					if ( ! empty( $b['blocked_by'] ) ) {
						...
						$who = sprintf( _x( 'blocked by %1$s at %2$s', 'e.g. blocked by John at 11:00', 'wp-cerber' ), $who, cerber_date( $b['blocked_time'] ) );
						echo ' - <i>' . $who . '</i>';
					}
					?>
				</label>
...
function crb_admin_show_sessions() {
	...
	if ( nexus_is_valid_request() ) {
		...
		add_filter(
			'set_url_scheme',
			function ( $url, $scheme, $orig_scheme ) {
...
function crb_pdata_exporter_act( $email_address, $page = 1 ) {
	...
	if ( ( ! $user = get_user_by( 'email', $email_address ) )
		...
		foreach ( $rows as $row ) {
			...
			$data[] = array(
				'name'  => cerber_date( $row->stamp, false ), // First column
