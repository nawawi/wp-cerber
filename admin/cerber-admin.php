 ...
 */
add_action(
	'wp_ajax_cerber_scan_bulk_files',
	function () {
...
function cerber_show_ignore() {
	...
	foreach ( $list as $key => $file ) {
		...
		$delete = '<a ' . $confirm . ' href="' . cerber_admin_link_add(
			array(
				'cerber_admin_do' => 'scan_tegrity',
...
function crb_scan_insights_brief( $scan_id ) {
	...
	return $result . cerber_make_table(
		$folders,
		array(
			__( 'Folder', 'wp-cerber' ),
			__( 'Path', 'wp-cerber' ),
		...
		'',
		'crb_align_right crb_align_left_2',
		'crb-monospace'
	);
...
function crb_scan_insights_exts( $scan_id ) {
	...
	$report .= cerber_make_table(
		...
		'crb-ext-statistics',
		'crb_align_right',
		'crb-monospace crb-anchor-decorated'
	);
			...
			if ( $data = crb_q_cache_get( 'SELECT COUNT(activity) as cnt, activity FROM ' . CERBER_LOG_TABLE . ' WHERE user_login = "' . $user->user_login . '" OR user_login = "' . $user->user_email . '" GROUP BY activity', CERBER_LOG_TABLE, $cache_only ) ) {
				foreach ( $data as $item ) {
					...
					$links[] = array(
						array(
							'filter_activity' => $item[1],
							'filter_login'    => $user->user_login . '|' . $user->user_email,
						),
						crb_array_get( $labels, $item[1], 'Unknown' ) . ' - ' . $item[0],
					);
				}
