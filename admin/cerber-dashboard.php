						...
						if ( $lic ) {
							if ( lab_validate_lic() ) {
								cerber_admin_message(
									'<b>Great! You have entered the valid license key.</b><p>Now, whenever you see a green shield icon in the top right-hand corner of any Cerber\'s admin page, it means the professional version works as intended, and your website is protected by WP Cerber Security Cloud.</p>
...
function crb_admin_get_tokenized_link() {
	...
	if ( cerber_update_set( '_the_key_' . $key, array( 'query' => nexus_request_data()->get_params ), 1, true, time() + 60 ) ) {
		return array(
			'redirect'     => true,
			'redirect_url' => home_url( '?cerber_magic_key=' . $key ),
		);
	}
		...
		foreach ( $rows as $row ) {
			...
			if ( empty( $args['no_details'] ) && $row->details ) {
				...
				if ( ( $uri = crb_array_get( $details, 4 ) )
					...
					$activity .= '<p class="crb_act_url" title="' . htmlspecialchars( $uri ) . '">' . str_replace(
						array( '-', '/' ),
						array(
							'<wbr>-',
							'<wbr>/',
...
function crb_generate_ip_insights( $ip, $tab = 'activity', $cache_only = false ) {
	...
	if ( $data = crb_q_cache_get( 'SELECT COUNT(activity) as cnt, activity FROM ' . CERBER_LOG_TABLE . ' WHERE ip = "' . $ip . '" GROUP BY activity', CERBER_LOG_TABLE, $cache_only ) ) {
		...
		foreach ( $data as $item ) {
			$links[] = array(
				array(
					'filter_activity' => $item[1],
					'filter_ip'       => $ip,
				),
				crb_array_get( $labels, $item[1], 'Unknown' ) . ' - ' . $item[0],
			);
		}
	}
...
function cerber_show_dashboard() {
	...
	if ( ! $locked ) {
		...
		$locked_ips = cerber_show_lockouts(
			array(
				'per_page' => 10,
				'no_navi'  => true,
			),
			...
			'visible'        =>
				array(
					'al_send_emails'     => array(
						...
						'atts'  => array(
							'type'                  => 'checkbox',
							'data-validation_group' => 'required_min_one',
							'checked'               => true,
						),
					),
...
function cerber_admin_head() {
	...
	<script type="text/javascript">
		...
		if ( cerber_is_admin_page( array( 'page' => 'cerber-integrity' ) ) ) {
			...
			echo 'crb_scan_msg_misc = ' . json_encode(
				array(
					'delete_file'     => array(
...
function cerber_show_traffic( $args = array(), $echo = true ) {
	...
	list( $rows, $total ) = crb_q_cache_get(
		array(
	...
	if ( $rows ) {
		...
		foreach ( $rows as $row ) {
			...
			$row_uri = str_replace(
				array( '-', '/', '%', '&', '=', '?', '(', ')' ),
				array( '<wbr>-', '<wbr>/', '<wbr>%', '<wbr>&', '<wbr>=', '<wbr>?', '<wbr>(', ')<wbr>' ),
				$row_uri
			);
	...
	if ( empty( $args['no_navi'] ) ) {
		...
		if ( ! $is_log_empty ) {
			...
			$nav_links[] = array(
				array(
					'filter_http_code'      => 399,
					'filter_http_code_mode' => 'GT',
				),
				__( 'Errors', 'wp-cerber' ),
			);
...
function crb_test_notify_link( $args = array(), $title = null ) {
	return '<span class="crb-insetting-link">[ <a href="' .
		...
		) . '">' . ( $title ?? __( 'Click to send test', 'wp-cerber' ) ) . '</a> ]</span>';
}
