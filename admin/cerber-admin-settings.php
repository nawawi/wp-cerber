...
function cerber_field_show( $config ) {
	...
	switch ( $config['type'] ) {
		...
		case 'attempts':
			$html = sprintf(
				__( '%1$s retries are allowed within %2$s minutes', 'wp-cerber' ),
		...
		case 'reglimit':
			$html = sprintf(
				__( '%1$s registrations are allowed within %2$s minutes from one IP address', 'wp-cerber' ),
		...
		case 'aggressive':
			$html = sprintf(
				__( 'Increase lockout duration to %1$s hours after %2$s lockouts in the last %3$s hours', 'wp-cerber' ),
		...
		case 'citadel':
			$html = sprintf(
				__( 'Enable after %1$s failed login attempts in the last %2$s minutes', 'wp-cerber' ),
...
*/
add_filter(
	'pre_update_option_' . CERBER_OPT,
	function ( $new, $old, $option ) {
...
*/
add_filter(
	'pre_update_option_' . CERBER_OPT_A,
	function ( $new, $old, $option ) {
...
*/
add_filter(
	'pre_update_option_' . CERBER_OPT_C,
	function ( $new, $old, $option ) {
...
*/
add_filter(
	'pre_update_option_' . CERBER_OPT_N,
	function ( $new, $old, $option ) {
...
*/
add_filter(
	'pre_update_option_' . CERBER_OPT_H,
	function ( $new, $old, $option ) {

		$new['restwhite'] = cerber_text2array(
			$new['restwhite'],
			"\n",
			function ( $v ) {
...
*/
add_filter(
	'pre_update_option_' . CERBER_OPT_T,
	function ( $new, $old, $option ) {
...
add_filter(
	'pre_update_option_' . CERBER_OPT_US,
	function ( $new, $old, $option ) {
...
add_filter(
	'pre_update_option_' . CERBER_OPT_OS,
	function ( $new, $old, $option ) {
...
*/
add_filter(
	'pre_update_option_' . CERBER_OPT_S,
	function ( $new, $old, $option ) {
...
*/
add_filter(
	'pre_update_option_' . CERBER_OPT_E,
	function ( $new, $old, $option ) {
...
add_filter(
	'pre_update_option_' . CERBER_OPT_P,
	function ( $new, $old, $option ) {
